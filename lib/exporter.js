...
var blockToHTML = exports.blockToHTML = function blockToHTML(block) {
  ...
  switch (blockType) {
    ...
    case _constants.Block.VIDEO:
      {
        var vidData = block.data;
        var _text = block.text;
        var _extraClass = _text.length > 0 ? ' md-block-image-has-caption' : '';
        return {
          start: '<figure class="md-block-video' + _extraClass + '"<video width="400" controls><source src="' + vidData.src + '" type="video/mp4" /></video><figcaption class="md-block-video-caption">',
          end: '</figcaption></figure>'
        };
      }
