...
var _reactAzureMp = require('react-azure-mp');
...
var blockToHTML = exports.blockToHTML = function blockToHTML(block) {
  ...
  var azure = null;
  if (blockType === _constants.Block.VIDEO) {
    azure = _react2.default.createElement(_reactAzureMp.AzureMP, { src: block.data.src });
  }
  ...
  switch (blockType) {
    ...
    case _constants.Block.VIDEO:
      {
        // const vidData = block.data;
        var _text = block.text;
        var _extraClass = _text.length > 0 ? ' md-block-image-has-caption' : '';
        return {
          start: '<figure class="md-block-image' + _extraClass + '">' + azure + '<figcaption class="md-block-image-caption">',
          end: '</figcaption></figure>'
        };
      }
